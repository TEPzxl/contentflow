...
| CI / release workflow | `.github/workflows/ci.yaml`、`.github/workflows/release-images.yaml` | 高 | workflow 调用 `scripts/ci.sh`，本轮未读取用户已修改的 `scripts/ci.sh` 内容 |
...
| “CI 覆盖 web-test 等” | workflow 调用 `scripts/ci.sh` | `scripts/ci.sh` 当前有用户未提交改动，本轮不读取其内容 | 不基于当前 `scripts/ci.sh` 细节下更强结论 |
...
- 不要说“DLQ 管理有管理员隔离”，`DLQHandler` 只检查登录态，列表和 replay 没有按 user scope 过滤。
